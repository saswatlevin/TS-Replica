...
describe('orderShippingAddressSchema - CITY Boundary Tests', () => {
    test('orderShippingAddressSchema - should accept city minimum length "Rio"', () => {
        // Arrange
        const testData = {
            address_type_id: "1",
        ...
        if (result?.error?.issues !== undefined){
            console.log("orderShippingAddressSchema - should accept city minimum length Rio - result?.error?.issues ", result?.error?.issues);
        }
        else {
            console.log("orderShippingAddressSchema - should accept city minimum length Rio - result ", result);
        }
    ...
    test('orderShippingAddressSchema - should accept city maximum length', () => {
        // Arrange
        ...
        if (result?.error?.issues !== undefined){
            console.log("orderShippingAddressSchema - should accept city maximum length - result?.error?.issues ", result?.error?.issues);
        }
        else {
            console.log("orderShippingAddressSchema - should accept city maximum length - result ", result);
        }
