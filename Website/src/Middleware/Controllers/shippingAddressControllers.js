...
const createShippingAddress = asyncErrorHandler(async (req, res, next) => {
    ...
    const result = await User.findOneAndUpdate({ user_id: user_id }, { $push: { ShippingAddresses: shipping_address } }, { new: true }, {runValidators: true}).lean(); // return the updated document;
...
const deleteShippingAddressById = asyncErrorHandler(async (req, res, next) => {
    ...
    const result = await User.updateOne({ user_id: user_id }, { $pull: { "ShippingAddresses": request_body_deep_clone } });
    console.log("result ", result);
