...
describe('productResponseSchema - HAPPY PATH TEST - product_fit', () => {
    test('should accept a valid product_fit of minimum length (1 character)', () => {
      ...
      const testData = {
        ...
        if (result?.error?.issues !== undefined){
          console.log("productResponseSchema - should accept a valid product_fit of minimum length (1 character) - product_fit - result?.error?.issues ", result?.error?.issues);
        }
            
        else {
          console.log("productResponseSchema - should accept a valid product_fit of minimum length (1 character) - product_fit - result ", result);
        }

        // Assert
        expect(result.success).toBe(true);
        expect(result.data.product_fit).toBe("B");
        expect(result.data.product_fit.length).toBe(1);
        expect(typeof result.data.product_fit).toBe('string');
  });
      ...
      const testData = {
          ...
          product_fit: "Élodie-François ångström Zürich Noël Björn García Søren Åsa Héloïse Mårten Dvok ukasz Þór Jón! The café’s façade gleamed—bright, calm, alive.Numbers danced: 12345, 67890; symbols sang: #!;%:.,“Quotes” and accents intertwined. À propos—of art, of science, of music—Øystein’s résumé stood tall. Máría whispered softly, “C’est la vie.” Bjørn replied, Indeed—it is.The harmony of words, punctuated by pau",
        ...
        if (result?.error?.issues !== undefined){
          console.log("productResponseSchema - should accept a valid product_fit of minimum length (400 characters) - product_fit - result?.error?.issues ", result?.error?.issues);
        }
            
        else {
          console.log("productResponseSchema - should accept a valid product_fit of minimum length (400 characters) - product_fit - result ", result);
        }

        // Assert
        expect(result.success).toBe(true);
        expect(result.data.product_fit).toBe("Élodie-François ångström Zürich Noël Björn García Søren Åsa Héloïse Mårten Dvok ukasz Þór Jón! The café’s façade gleamed—bright, calm, alive.Numbers danced: 12345, 67890; symbols sang: #!;%:.,“Quotes” and accents intertwined. À propos—of art, of science, of music—Øystein’s résumé stood tall. Máría whispered softly, “C’est la vie.” Bjørn replied, Indeed—it is.The harmony of words, punctuated by pau");
        expect(result.data.product_fit.length).toBe(400);
        expect(typeof result.data.product_fit).toBe('string');
  });
});