...
const updateShippingAddress = asyncErrorHandler(async (req, res, next) => {
    ...
    const update_request = Object.fromEntries(Object.entries(request_body).map(([key, value]) => [`ShippingAddresses.$.${key}`, value]));
    ...
    // A live mongoose document contains many hiiden keys and fields. 
    // The .lean() functions strips these keys and fields completely.
    const result = await User.updateOne(shipping_address_selector, {
        $set: update_request
    }).lean();
...
const searchShippingAddress = asyncErrorHandler(async (req, res, next) => {
    ...
    const shipping_address_search_query = Object.fromEntries(Object.entries(request_body).map(([key, value]) => [`ShippingAddresses.${key}`, value]));
