...
describe('searchShippingAddressRequestSchema - APARTMENT Boundary Tests', () => {
    test('searchShippingAddressRequestSchema - should accept apartment minimum length ""', () => {
        // Arrange
        const testData = {
        ...
        if (result?.error?.issues !== undefined){
            console.log("searchShippingAddressRequestSchema - should accept apartment minimum length - result?.error?.issues ", result?.error?.issues);
        }
        else {
            console.log("searchShippingAddressRequestSchema - should accept apartment minimum length - result ", result);
        }
    ...
    test('searchShippingAddressRequestSchema - should accept apartment maximum length', () => {
        // Arrange
        const testData = {
            _id: new mongoose.Types.ObjectId("64b64c4f5311236168a109ca"),
            shipping_address_id: "1sjkopnytguq",
        ...
        if (result?.error?.issues !== undefined){
            console.log("searchShippingAddressRequestSchema - should accept apartment maximum length - result?.error?.issues ", result?.error?.issues);
        }
        else {
            console.log("searchShippingAddressRequestSchema - should accept apartment maximum length - result ", result);
        }
