...
describe('updateProductSchema - HAPPY PATH TEST - supply_type_description', () => {
    test('should validate a product request with valid supply_type_description  - TEST 1', () => {
        ...
        const testData = {
            ...
            if (result?.error?.issues !== undefined){
                console.log("updateProductSchema - should accept valid supply_type - TEST 1 - supply_type - result?.error?.issues ", result?.error?.issues);
            }
            
            else {
                console.log("updateProductSchema - should accept valid supply_type - TEST 1 - supply_type - result ", result);
            }
    ...
    test('should validate a product request with valid supply_type_description  - TEST 2', () => {
        ...
        const testData = {
            ...
            const result = updateProductSchema.safeParse(testData);
            if (result?.error?.issues !== undefined){
                console.log("updateProductSchema - should accept valid supply_type - TEST 2 - supply_type - result?.error?.issues ", result?.error?.issues);
            }
            
            else {
                console.log("updateProductSchema - should accept valid supply_type - TEST 2 - supply_type - result ", result);
            }
