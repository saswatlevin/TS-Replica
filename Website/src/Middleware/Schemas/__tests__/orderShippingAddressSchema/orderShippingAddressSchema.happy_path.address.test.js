...
describe('orderShippingAddressSchema - ADDRESS Tests', () => {
    test('orderShippingAddressSchema - should accept address minimum length "7"', () => {
        // Arrange
        const testData = {
            address_type_id: "1",
        ...
        if (result?.error?.issues !== undefined){
            console.log("orderShippingAddressSchema - should accept address minimum length 7 - result?.error?.issues ", result?.error?.issues);
        }
        else {
            console.log("orderShippingAddressSchema - should accept address minimum length 7 - result ", result);
        }
    ...
    test('orderShippingAddressSchema - should accept address maximum length', () => {
        // Arrange
        const testData = {
            address_type_id: "1",
            company_name: "Brooklinen, Inc",
        ...
        if (result?.error?.issues !== undefined){
            console.log("orderShippingAddressSchema - should accept address maximum length - result?.error?.issues ", result?.error?.issues);
        }
        else {
            console.log("orderShippingAddressSchema - should accept address maximum length - result ", result);
        }
