...
describe('shippingAddressStandardSchema - Happy Path Tests', () => {
    test('shippingAddressStandardSchema - should accept valid shipping address data set 1', () => {
        // Arrange
        const testData = {
        ...
        if (result?.error?.issues !== undefined){
            console.log("shippingAddressStandardSchema - should accept valid shipping address data set 1 - result?.error?.issues ", result?.error?.issues);
        }
        else {
            console.log("shippingAddressStandardSchema - should accept valid shipping address data set 1 - result ", result);
        }
    ...
    test('shippingAddressStandardSchema - should accept valid shipping address data set 2', () => {
        // Arrange
        const testData = {
            _id: new mongoose.Types.ObjectId("656f7c9a8b3e4f1d2a7b9c0e"),
            shipping_address_id: "1sjkopnytguq",
        ...
        if (result?.error?.issues !== undefined){
            console.log("shippingAddressStandardSchema - should accept valid shipping address data set 2 - result?.error?.issues ", result?.error?.issues);
        }
        else {
            console.log("shippingAddressStandardSchema - should accept valid shipping address data set 2 - result ", result);
        }
