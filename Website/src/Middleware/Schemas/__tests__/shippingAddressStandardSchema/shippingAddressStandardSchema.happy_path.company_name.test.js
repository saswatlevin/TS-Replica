...
describe('shippingAddressStandardSchema - COMPANY_NAME Boundary Tests', () => {
    test('shippingAddressStandardSchema - should accept company_name minimum length ""', () => {
        // Arrange
        const testData = {
        ...
        if (result?.error?.issues !== undefined){
            console.log("shippingAddressStandardSchema - should accept company_name minimum length - result?.error?.issues ", result?.error?.issues);
        }
        else {
            console.log("shippingAddressStandardSchema - should accept company_name minimum length - result ", result);
        }
    ...
    test('shippingAddressStandardSchema - should accept company_name maximum length', () => {
        // Arrange
        const testData = {
            _id: new mongoose.Types.ObjectId("64b64c4f5311236168a109ca"),
            shipping_address_id: "1sjkopnytguq",
        ...
        if (result?.error?.issues !== undefined){
            console.log("shippingAddressStandardSchema - should accept company_name maximum length - result?.error?.issues ", result?.error?.issues);
        }
        else {
            console.log("shippingAddressStandardSchema - should accept company_name maximum length - result ", result);
        }
